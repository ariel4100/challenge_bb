...
const getAll = async () => {
    try {
        const { data } = await axios.get("https://json-biglifeapp.herokuapp.com/activity");        
        totalPage.value = Math.round((data.length / limit));
    } catch (error) {
        console.log(error);
    }
};
...
const getData = async () => {
    try {
        const { data } = await axios.get(`https://json-biglifeapp.herokuapp.com/activity?_page=${currentPage.value}&_limit=${limit}`);
...
<template>
  <div class="md:px-10 py-20" v-if="activities.length">
    <div class="container mx-auto px-4">
      ...
      <div class="text-center">
        <pagination :pages="totalPage" :currentPage="currentPage" @page-changed="info"></pagination>
      </div>
    </div>
  </div>
